...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/jmpezzHW5"
## Provided:
...
def delete(lst):
    ...
    delete_lst = TodoList.query.filter_by(title = lst).first()
    if delete_lst:
        db.session.delete(delete_lst)
        flash('Deleted list: {}'.format(delete_lst.title))
    return redirect(url_for('all_lists'))
